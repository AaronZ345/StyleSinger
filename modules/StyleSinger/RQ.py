...
class RQBottleneck(nn.Module):
    ...
    Arguments:
        latent_shape (Tuple[ int, int]): the shape of latents, denoted (H, D)
        code_shape (Tuple[ int, int]): the shape of codes, denoted (h, d)
    ...
    def __init__(self,
        ...
        if not len(code_shape) == len(latent_shape) == 2:
            raise ValueError("incompatible code shape or latent shape")
        if any([y % x != 0 for x, y in zip(code_shape[:-1], latent_shape[:-1])]):
            raise ValueError("incompatible code shape or latent shape")

        #residual quantization does not divide feature dims for quantization.
        embed_dim = np.prod(latent_shape[:1]) // np.prod(code_shape[:1]) * latent_shape[1]
        ...
        self.restart_unused_codes = restart_unused_codes
        self.n_embed = n_embed if isinstance(n_embed, Iterable) else [n_embed for _ in range(rq_depth)]
        self.decay = decay if isinstance(decay, Iterable) else [decay for _ in range(rq_depth)]
        assert len(self.n_embed) == rq_depth
        assert len(self.decay) == rq_depth
        ...
        if self.shared_codebook:
            ...
            self.codebooks = nn.ModuleList([codebook0 for _ in range(rq_depth)])
        else:
            codebooks = [VQEmbedding(self.n_embed[idx], 
                                     embed_dim, 
                                     decay=self.decay[idx], 
                                     restart_unused_codes=restart_unused_codes,
                                     ) for idx in range(rq_depth)]
    ...
    def forward(self, x):
        ...
        return quants_trunc, commitment_loss
